...
(add-hook! 'kizen-test-mode-hook (modify-syntax-entry ?_ "w"))
#+END_SRC
...
(defun comint-set-cursor-control ()
  "Set the necessary cursor control in comint mode."
  ...
  (add-hook 'comint-preoutput-filter-functions
            'xterm-color-filter)
  ;; (add-hook 'comint-output-filter-functions
  ;;           'comint-remove-cpr-warning)
  )

(defun comint-remove-cpr-warning (output)
  "Remove CPR warning message from comint OUTPUT."
  (let ((cpr-warning "WARNING: your terminal doesn't support cursor position requests (CPR)."))
    (if (string-match-p cpr-warning output)
        (replace-regexp-in-string cpr-warning "" output)
      output)))

(add-hook! 'comint-mode-hook 'comint-set-cursor-control)
#+end_src
